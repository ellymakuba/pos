...
class Receiving_lib
{
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null,$bonus=0)
	{
	...
	function edit_item($line,$description,$serialnumber,$quantity,$discount,$price,$bonus)
	{
		$items = $this->get_cart();
		if(isset($items[$line]))
		{
	...
	function copy_entire_receiving($receiving_id)
	{
		...
		foreach($this->CI->Receiving->get_receiving_items($receiving_id)->result() as $row)
		{
			$this->add_item($row->item_id,$row->quantity_purchased,$row->discount_percent,$row->item_unit_price,$row->description,$row->serialnumber,$row->bonus);
		}
		$this->set_supplier($this->CI->Receiving->get_supplier($receiving_id)->person_id);
