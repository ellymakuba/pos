...
{	
	...
	function inventory_summary($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$total_value=+$total_value+($row['quantity']*$row['cost_price']);
			$tabular_data[] = array($row['name'], $row['cost_price'],$row['unit_price'],$row['quantity'], $row['reorder_level'],$row['quantity']*$row['cost_price']);
		}
